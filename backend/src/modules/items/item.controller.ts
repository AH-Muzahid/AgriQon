...
export const itemController = {
  ...
  async create(req: Request, res: Response) {
    ...
    const item = await itemService.create(req.user!.id, payload);
    return res.status(201).json(item);
  },

  async update(req: Request, res: Response) {
    const payload = updateItemSchema.parse(req.body);
    const result = await itemService.update(req.params.id, req.user!.id, req.user!.role === Role.ADMIN, payload);
  ...
  async remove(req: Request, res: Response) {
    const result = await itemService.remove(req.params.id, req.user!.id, req.user!.role === Role.ADMIN);
